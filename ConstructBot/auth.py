...
async def get_pyrogram_accounts():
    ...
    pyrogram_accounts = query.filter(models.Utm.is_bot_person == True).all()
    session.close()
    return pyrogram_accounts
...
for account in accounts:
    ...
    async def aiogram_bot():
        ...
        async def code_broker(message: types.Message, state: FSMContext):
            ...
            try:
                ...
                await message.answer(f"❌ Error: make sure is correct and send again or {e}")
    ...
    async def pyrogram_provoke_auth():
        ...
        client = Client(
            name=f"src/pyrogram/sessions/{account.phone}",
    ...
    # AUTHENTICATING FOR EACH BOT (AIOGRAM + PYROGRAM)
